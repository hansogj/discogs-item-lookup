...
export async function lookupRelease(releaseId: string, discogsToken?: string): Promise<LookupResult> {
  ...
  if (!master_id) {
     throw new DiscogsApiError(`This release (ID: ${releaseId}) is not associated with a master release, so the original year cannot be determined.`);
  }

  // 2. Fetch the master release data to get the original year
